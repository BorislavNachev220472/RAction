...
for folder in os.listdir(input_path):
    if ".DS_Store" not in folder:
        new_input_path = os.path.join(input_path, folder)
        new_output_path = os.path.join(output_path, folder)
        os.makedirs(new_output_path, exist_ok=True)
        for filename in os.listdir(new_input_path):
            if ".DS_Store" not in filename:
                # Clear existing objects in the scene
                bpy.ops.object.select_all(action='SELECT')
                bpy.ops.object.delete()

                file_path = os.path.join(new_input_path, filename)
                print(f"Processing: {file_path}")

                fragment_name = file_path.split('/')[-1].split('.')[0]
                fragment_dupe_name = fragment_name + ".001"

                os.makedirs(os.path.join(new_output_path, f"ground_truth"), exist_ok=True)
                gt_path = os.path.join(new_output_path, f"ground_truth/{fragment_name}_ground_truth.stl")
                agent_path = os.path.join(new_output_path, f"{fragment_name}_agent.stl")
                base_path = os.path.join(new_output_path, f"{fragment_name}_base.stl")
                distance_path = os.path.join(new_output_path, f"{fragment_name}_distances.npy")
