...
import numpy as np
import trimesh
from custom_enviroment.mesh_utils import create_mesh, apply_rotation_mesh, get_voxelgrid

class Simulation:
    def __init__(self, render, voxel_shape=(256, 256, 256)):
        if render:
            mode = p.GUI
    ...
    def reset(self):
        ...
        for i in range(self.number_of_agents):
            ...
            mesh = create_mesh(agent_file_path)
            mesh = apply_rotation_mesh(mesh, [0,0,0], orentation, cordinates)
            ...
            sample_vertices = random.sample(mesh_vertices, 50)
            for vertice in sample_vertices:
        ...
        voxelized_mesh = get_voxelgrid(combined_mesh, self.voxel_size, self.voxel_shape[0])
    ...
    def run(self, actions):
        ...
        for i in range(self.number_of_agents):
            ...
            previous_cordinate = agent["cordinates"]
            previous_euler = agent["euler"]
            previous_orentation = p.getQuaternionFromEuler(previous_euler)

        
            cordinates, orentation = p.getBasePositionAndOrientation(agent_object_id)
            euler = p.getEulerFromQuaternion(orentation)

            orentation_difference = p.getDifferenceQuaternion(previous_orentation, orentation)
            mesh = apply_rotation_mesh(agent_mesh, previous_cordinate, orentation_difference, cordinates)
